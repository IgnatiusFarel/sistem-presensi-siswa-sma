<template>
  ...
  <n-modal
    ...
    content="Apakah Anda yakin ingin menghapus data berita ini?"
...
const handleDelete = async () => {
  ...
  try {
    if (Array.isArray(deleteTarget.value)) {    
      await Api.delete('/daftar-berita', {
        data: { ids: deleteTarget.value },
      });
    } else {      
      await Api.delete(`/daftar-berita${deleteTarget.value}`);
    }
    message.success('Data berita berhasil dihapus!');
    await fetchData();
    selectedRows.value = [];
  } catch (error) {
    message.error('Data berita gagal dihapus!');
    console.error(error);
  } finally {
    loading.value = false;
  }
...
const fetchData = async () => {
  loading.value = true;
  try {
    const response = await Api.get("/daftar-berita");
